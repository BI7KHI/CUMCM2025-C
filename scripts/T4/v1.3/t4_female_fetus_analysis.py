...
# ---------------------- 配置 ----------------------
# 使用相对脚本位置动态推导项目根目录，避免硬编码绝对路径
BASE_DIR = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..'))
SCRIPT_VERSION = 'v1.3'
# 数据与结果目录（默认输出到 results/T4/v1.3）
...
def main(target_recall: float = 0.90, test_size: float = 0.2, seed: int = 42,
    ...
    # 保存女胎筛选统计，增强可追溯性
    try:
        with open(os.path.join(RESULTS_DIR, 'female_selection_stats.json'), 'w', encoding='utf-8') as fs:
            json.dump({'version': SCRIPT_VERSION, **female_stats}, fs, ensure_ascii=False, indent=2)
    except Exception as e:
        print('Warning: failed to persist female selection stats:', e)
    ...
    param_grid = [
        {'clf__solver': ['liblinear'], 'clf__penalty': ['l1','l2'], 'clf__C': [0.05, 0.1, 0.3, 1.0, 3.0, 10.0]},
        {'clf__solver': ['saga'],      'clf__penalty': ['l1','l2'], 'clf__C': [0.05, 0.1, 0.3, 1.0, 3.0, 10.0]}
    ]
    ...
    # 持久化CV搜索产物（便于论文与复现实验）
    try:
        with open(os.path.join(RESULTS_DIR, 'cv_best_params.json'), 'w', encoding='utf-8') as fp:
            json.dump({'version': SCRIPT_VERSION, 'best_params': gs.best_params_, 'best_score': float(gs.best_score_)}, fp, ensure_ascii=False, indent=2)
        cv_df = pd.DataFrame(gs.cv_results_)
        cv_df.to_csv(os.path.join(RESULTS_DIR, 'cv_results.csv'), index=False, encoding='utf-8-sig')
    except Exception as e:
        print('Warning: failed to persist CV results:', e)
    ...
    if calibrate and 0.0 < calib_size < 0.9:
        ...
        # sklearn>=1.6 使用 estimator 参数名
        best_model = CalibratedClassifierCV(estimator=base, cv='prefit', method=calib_method)
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, 'roc_curve_test.svg'))
    plt.close()
    # 混淆矩阵（F1最优）
    ...
    try:
        ...
        with open(os.path.join(RESULTS_DIR, 'thresholds.json'), 'w', encoding='utf-8') as tf:
            json.dump({'version': SCRIPT_VERSION,
                       'best_threshold_f1': float(best_thr),
                       'recall_priority_threshold': float(rec90_thr),
                       'target_recall': float(target_recall)}, tf, ensure_ascii=False, indent=2)
    except Exception as e:
        print('Warning: failed to persist model or thresholds:', e)

    # 保存特征列表、运行配置与分类报告（便于论文复现）
    try:
        with open(os.path.join(RESULTS_DIR, 'feature_list.json'), 'w', encoding='utf-8') as ff:
            json.dump({'version': SCRIPT_VERSION, 'features': list(X.columns)}, ff, ensure_ascii=False, indent=2)
        with open(os.path.join(RESULTS_DIR, 'run_config.json'), 'w', encoding='utf-8') as rc:
            json.dump({'version': SCRIPT_VERSION,
                       'seed': int(seed), 'test_size': float(test_size), 'target_recall': float(target_recall),
                       'calibrate': bool(calibrate), 'calib_size': float(calib_size), 'calib_method': str(calib_method)}, rc, ensure_ascii=False, indent=2)
        with open(os.path.join(RESULTS_DIR, 'classification_report_f1.txt'), 'w', encoding='utf-8') as cr1:
            cr1.write(metrics['classification_report'])
        with open(os.path.join(RESULTS_DIR, 'classification_report_recall_priority.txt'), 'w', encoding='utf-8') as cr2:
            cr2.write(metrics_rec90['classification_report'])
    except Exception as e:
        print('Warning: failed to persist reports/config/features:', e)

    with open(os.path.join(RESULTS_DIR, 'metrics_f1.json'), 'w', encoding='utf-8') as jf:
        json.dump({'version': SCRIPT_VERSION, **{k:v for k,v in metrics.items() if k!='classification_report'}}, jf, ensure_ascii=False, indent=2)
    with open(os.path.join(RESULTS_DIR, 'metrics_recall_priority.json'), 'w', encoding='utf-8') as jf:
        json.dump({'version': SCRIPT_VERSION, **{k:v for k,v in metrics_rec90.items() if k!='classification_report'}}, jf, ensure_ascii=False, indent=2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--results_dir', type=str, default=None, help=f'结果输出目录，默认 {DEFAULT_RESULTS_DIR}')
