...
# 男胎X染色体浓度分布
sns.kdeplot(male_fetus_data['X染色体浓度'].dropna(), label='男胎', shade=True, alpha=0.5)
# 女胎X染色体浓度分布
sns.kdeplot(female_fetus_data['X染色体浓度'].dropna(), label='女胎', shade=True, alpha=0.5)

plt.title('男胎和女胎的X染色体浓度分布')
plt.xlabel('X染色体浓度')
plt.ylabel('密度')
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(results_dir, 'T3_x_chromosome_distribution.png'), dpi=300)
plt.close()
...
if hasattr(best_optimized_model, 'feature_importances_'):
    ...
    plt.title(f'{best_model_name}的特征重要性')
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, f'T3_feature_importance_{best_model_name}.png'), dpi=300)
    plt.close()
    ...
    # 按绝对值排序
    feature_coef_df['绝对值'] = feature_coef_df['系数'].abs()
    feature_coef_df = feature_coef_df.sort_values('绝对值', ascending=False)
    ...
    # 可视化特征系数
    plt.figure(figsize=(12, 8))
    sns.barplot(x='系数', y='特征', data=feature_coef_df)
    plt.title(f'{best_model_name}的特征系数')
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, f'T3_feature_coef_{best_model_name}.png'), dpi=300)
    plt.close()
...
def plot_metrics(y_true, y_pred_proba, model_name, save_dir):
    if y_pred_proba is not None:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f'T3_roc_curve_{model_name}.png'), dpi=300)
        plt.close()
        ...
        plt.xlabel('召回率')
        plt.ylabel('精确率')
        plt.title(f'{model_name}的PR曲线')
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f'T3_pr_curve_{model_name}.png'), dpi=300)
        plt.close()
...
plt.title('X染色体浓度异常与其他染色体异常的相关性')
plt.tight_layout()
plt.savefig(os.path.join(results_dir, 'T3_correlation_heatmap.png'), dpi=300)
plt.close()
...
results_df.to_excel(os.path.join(results_dir, 'T3_model_evaluation_results.xlsx'), index=False)

# 保存最佳模型的特征重要性或系数
if 'feature_importance_df' in locals():
    feature_importance_df.to_excel(os.path.join(results_dir, 'T3_feature_importance.xlsx'), index=False)
elif 'feature_coef_df' in locals():
    feature_coef_df.to_excel(os.path.join(results_dir, 'T3_feature_coefficients.xlsx'), index=False)

# 保存相关性矩阵
correlation_matrix.to_excel(os.path.join(results_dir, 'T3_correlation_matrix.xlsx'))
