...
RESULTS_DIR = os.path.join(BASE_DIR, "results_t3_v1.4_deep_optimized")
...
def train_evaluate_model(df):
    ...
    report_path = os.path.join(RESULTS_DIR, "T3_model_evaluation_report_SMOTE.txt")
...
def dynamic_bmi_clustering(df):
    ...
    plt.grid(True)
    plt.savefig(os.path.join(RESULTS_DIR, 'T3_silhouette_scores_for_bmi.png'))
    plt.close()
    ...
    report_path = os.path.join(RESULTS_DIR, "T3_bmi_group_report.txt")
    with open(report_path, "w", encoding='utf-8-sig') as f:
...
def main():
    ...
    plt.savefig(os.path.join(RESULTS_DIR, 'T3_risk_curves_deep_optimized.png'))
    plt.close()
    
    # 保存最终报告
    report_path = os.path.join(RESULTS_DIR, "T3_optimal_timing_report_deep_optimized.txt")
    with open(report_path, "w", encoding='utf-8-sig') as f:
