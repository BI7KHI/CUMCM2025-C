...
plt.legend()
plt.tight_layout()
plt.savefig(os.path.join(results_dir, 'T3_beta_v1.0.0.png'), dpi=300)
plt.close()
...
if hasattr(best_optimized_model, 'feature_importances_'):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, f'T3_beta_v1.0.0{best_model_name}.png'), dpi=300)
    plt.close()
...
def plot_metrics(y_true, y_pred_proba, model_name, save_dir):
    if y_pred_proba is not None:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_dir, f'T3_beta_v1.0.0{model_name}.png'), dpi=300)
        plt.close()
...
results_df.to_excel(os.path.join(results_dir, 'T3_beta_v1.0.0.xlsx'), index=False)

# 保存最佳模型的特征重要性或系数
if 'feature_importance_df' in locals():
    feature_importance_df.to_excel(os.path.join(results_dir, 'T3_beta_v1.0.0.xlsx'), index=False)
elif 'feature_coef_df' in locals():
    feature_coef_df.to_excel(os.path.join(results_dir, 'T3_beta_v1.0.0.xlsx'), index=False)

# 保存相关性矩阵
correlation_matrix.to_excel(os.path.join(results_dir, 'T3_beta_v1.0.0.xlsx'))
