...
# 创建结果目录
os.path.join(project_root, 'results', 'T3', 'alpha', 'v1.0')
...
class T3EnhancedAnalysis:
    ...
    def regression_modeling(self):
        ...
        # 保存模型结果
        with open(os.path.join(results_dir, 'T3_alpha_v1.0.0.txt'), 'w', encoding='utf-8') as f:
    ...
    def generate_report(self):
        ...
        # 保存报告
        with open(os.path.join(results_dir, 'T3_alpha_v1.0.0.md'), 'w', encoding='utf-8') as f:
            f.write('\n'.join(report))
        
        print("分析报告已保存到:", os.path.join(results_dir, 'T3_alpha_v1.0.0.md'))
