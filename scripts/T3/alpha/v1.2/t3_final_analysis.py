...
def main():
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'T3_alpha_v1.2.2.2.2_Y染色体浓度分析.png'), 
...
*报告生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*
    ...
    # 保存报告
    with open(os.path.join(results_dir, 'T3_alpha_v1.2.2.2.2_Y染色体分析报告.md'), 'w', encoding='utf-8') as f:
        f.write(report)
