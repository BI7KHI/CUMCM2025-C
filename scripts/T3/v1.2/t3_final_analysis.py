...
def main():
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(results_dir, 'T3_v12_Y染色体浓度分析.png'), 
...
*报告生成时间: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}*
    ...
    # 保存报告
    with open(os.path.join(results_dir, 'T3_v12_Y染色体分析报告.md'), 'w', encoding='utf-8') as f:
        f.write(report)
    
    print(f"综合分析报告已保存到: {results_dir}")
    print("✅ T3 v1.2 Y染色体浓度达标时间分析完成！")
